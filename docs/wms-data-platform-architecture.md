...
> transformações dbt, orquestração Airflow, enriquecimento geográfico via APIs públicas,
...
└─────────────────────────────────┬───────────────────────────────────────┘
                                  ...
                                  ▼
┌─────────────────────────────────────────────────────────────────────────┐
│              TRANSFORMATION LAYER — dbt Core + Amazon Athena            │
│                                                                          │
...
│   ├── modules/athena             workgroup + Glue catalog               │
...
│   └── dbt_wms/
│       ├── dbt_project.yml
│       ├── profiles.yml              ← Athena + Redshift targets
│       ├── models/
│       │   ├── staging/
│       │   ├── intermediate/
│       │   └── marts/
...
│   │   ├── 003-athena-vs-redshift-transform.md
...
| `wms-athena-results-{env}` | resultados queries | lifecycle 7 dias |
...
| Recurso | Uso | Custo |
|---|---|---|
| Athena workgroup | dbt transformations | centavos/query |
...
| Transformação | dbt Core + Amazon Athena |
...
|---|---|---|
| 001 | Iceberg no lugar de Delta Lake | Integração nativa AWS: Athena, Glue, Redshift Spectrum |
| 002 | DMS CDC no lugar de extração por timestamp | Captura deletes, múltiplos updates, sem dependência de `updated_at` |
| 003 | Athena para transformação, Redshift para serving | Athena serverless para dbt, Redshift otimizado para queries analíticas |
